...
export function registerReactionForOperation(
  ...
  let { target, key, type } = operation as Required<Operation>
  ...
  // 拿到原始对象 -> 观察者的map
  const reactionsForRaw = connectionStore.get(target) as Required<ReactionForRaw>
  // 拿到key -> 观察者的set
  ...
  if (!reactionsForKey.has(reaction)) {
    ...
    reaction.cleaners && reaction.cleaners.push(reactionsForKey)
  }
}
 ...
 */
export function getReactionsForOperation(operation: Operation) {
  let { target, key, type } = operation as  Required<Operation>
  // 拿到原始对象 -> 观察者的map
  const reactionsForTarget = connectionStore.get(target) as ReactionForRaw
