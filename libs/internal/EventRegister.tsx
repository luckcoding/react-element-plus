...
const hasRegistered = ({ id }) => not_null(registerMap.ids[id]);

const cleanRegister = (props) => {
  const {
    target, eventName, func, isUseCapture, id,
  } = props;
...
const doRegister = (props) => {
  const {
    id, eventName, func, isUseCapture,
  } = props;
 ...
 */
export default class EventRegister extends Component<any> {
  componentDidMount() {
